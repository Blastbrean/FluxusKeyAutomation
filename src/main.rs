...
async fn main() -> Result<(), reqwest::Error> {
    ...
    if res_check_final == "Trying to bypass the Fluxus key system will get you banned from using Fluxus." {
    ...
    for line in res_check_final.lines() {
        ...
        // print key...
        println!("here is your key: {}", line.replace("let content = (\"", "").replace("\");", "").trim());
        pause();
    ...
    // failed to find key...
    println!("failed to find key, report this output to blastbrean on discord: {}", res_check_final);
    pause();
